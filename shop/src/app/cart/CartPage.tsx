...
const TRUST = [
  {
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M5 8h14M5 8a2 2 0 1 0 0-4h14a2 2 0 1 0 0 4M5 8v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V8M10 12h4" />
      </svg>
    ),
    label: "Free Shipping",
  ...
  },
  {
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M16 15v1a3 3 0 0 1-3 3H5a3 3 0 0 1-3-3V7a3 3 0 0 1 3-3h8a3 3 0 0 1 3 3v1M21 12H9m0 0 3-3m-3 3 3 3" />
      </svg>
    ),
  ...
  return (
    <div className="flex gap-5 py-6 border-b border-gray-100 last:border-0">
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between gap-3">
        <div>
          ...
          <Link href={`/products/${item.slug}`}>
            <p className="text-sm sm:text-base font-semibold text-gray-900 hover:text-primary transition-colors line-clamp-2 leading-snug">
              {item.name}
            </p>
          </Link>
          {item.color && (
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="text-right">
            ...
            {item.originalPrice && (
              <p className="text-xs text-gray-400 line-through">${(item.originalPrice * item.qty).toFixed(2)}</p>
            )}
            {saving > 0 && (
              <p className="text-xs text-emerald-600 font-semibold">Save ${saving.toFixed(2)}</p>
            )}
          </div>
        </div>
      </div>
      ...
      <button
        ...
        className="self-start p-2 rounded-xl text-gray-300 hover:text-red-500 hover:bg-red-50 transition-colors mt-1 shrink-0"
      ...
      {items.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          <div className="w-24 h-24 rounded-full bg-gray-100 flex items-center justify-center mb-6">
            <svg className="w-11 h-11 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          <p className="text-gray-500 max-w-sm mb-8">
            Looks like you haven&apos;t added anything yet. Browse our collections to find something you&apos;ll love.
          </p>
          <Link
            href="/collections"
            className="inline-flex items-center gap-2 px-8 py-3.5 bg-primary text-white font-bold rounded-full hover:bg-primary-hover transition-colors shadow-sm"
          >
            Shop Collections
        ...
        <div className="grid lg:grid-cols-[1fr_380px] gap-8 items-start">
          ...
          <div className="sticky top-24 space-y-4">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
              ...
              {/* Checkout btn */}
              <button className="w-full py-4 bg-primary text-white font-bold rounded-full hover:bg-primary-hover transition-colors shadow-sm text-sm flex items-center justify-center gap-2">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <div className="flex items-center justify-center gap-2 flex-wrap pt-1">
                {["VISA", "MC", "AMEX", "PAYPAL", "APPLE"].map((p) => (
                  <span key={p} className="px-2.5 py-1 bg-gray-100 rounded text-[9px] font-bold text-gray-500 tracking-widest">
                    {p}
                  </span>
                ))}
              </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="flex gap-2">
                ...
                />
                <button className="px-5 py-2.5 bg-gray-900 text-white text-sm font-semibold rounded-full hover:bg-gray-700 transition-colors">
                  Apply
                </button>
              </div>
            </div>
