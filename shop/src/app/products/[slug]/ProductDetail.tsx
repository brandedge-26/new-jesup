...
function ShareModal({ product, onClose }: { product: Product; onClose: () => void }) {
  ...
  function nativeShare() {
    if (navigator.share) {
      navigator.share({ title: product.name, text: `Check out ${product.name} on Jesup Shop!`, url });
    }
  }
  ...
  const socials = [
    ...
    },
    {
      ...
      href: `https://twitter.com/intent/tweet?text=${encodeURIComponent(`Check out ${product.name}`)}&url=${encodeURIComponent(url)}`,
      icon: <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.737-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z" />,
    },
    {
      label: "Facebook",
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-7">
        ...
        <div className="flex gap-2 mb-4">
          <div className="flex-1 px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs text-gray-400 truncate select-all">
            {url}
          </div>
          <button
            onClick={copyLink}
        ...
        {/* Social share */}
        <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-3">Share on</p>
        <div className="flex gap-2">
          {socials.map((s) => (
            <a
              ...
              title={s.label}
              className={`flex-1 flex items-center justify-center py-2.5 rounded-xl border border-gray-200 text-gray-500 transition-all ${s.color}`}
            >
          ...
          {typeof navigator !== "undefined" && "share" in navigator && (
            <button
              onClick={nativeShare}
              title="More options"
              className="flex-1 flex items-center justify-center py-2.5 rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-900 transition-all"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button
                ...
                className="flex items-center justify-center w-13 h-13 rounded-full border-2 border-gray-200 text-gray-400 hover:border-primary hover:text-primary transition-all px-3.5 py-3.5"
